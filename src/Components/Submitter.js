...
const Submitter = () => {
    ...
    const fetchHistoricalData = async (symbol) => {
        try {
            let response = await axios.get(`https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY&symbol=${symbol}&apikey=9k_njIQwSUl2Qb2JWG_IZuIC85DAnJ1d`);
    ...
    useEffect(() => {
        ...
        const updateInterval = setInterval(updateWatchlist, 60000);
    ...
    return (
        <div className='my-4 '>
            <form className="d-flex " role="search" onSubmit={handleSubmit} style={{height:'40px',borderRadius:'0px'}}>
                <input
                    className="form-control me-2 glowing-txt glowing-btn"
                ...
                <div className="mt-3">
                    <div className='watchlist-container'>
                        {watchlist.map((stock, index) => (
                            <div className="watchItem" style={{backgroundColor:'white',opacity:'0.8',border:'blue'}} key={index}>
                                <strong>{stock.symbol}: US${stock.price} </strong>({parseFloat(stock.percentChange) > 0 ? <span style={{ color: 'green' }}>▲</span> : <span style={{ color: 'red' }}>▼</span>}{' '}
